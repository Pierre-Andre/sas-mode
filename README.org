...
** Installation
   ...
   2. To use SAS mode, add the following to your
      =.emacs=
...
2. Submit commands
   ...
   (defvar sas-mode-map
    ...
    "Keymap for `sas-mode'.")
   #+end_src
   - For instance a =C-RET= control-return (or invocation of =sas-shell-send-dwim=) in a middle of a data step or a proc step will send the whole data step or proc step to SAS.
   ...
   - =C-c C-q= will close the SAS buffer
** Config
- The custom variable =sas-shell-interpreter= is  =sas= but it can be useful to point to another sas executable.
- The custom =sas-sas-windows= can be set to =t= if SAS on windows is used. For windows user it is not possible to run SAS in Interactive Line Mode and flags are differents.
...
- For [[https://github.com/hlissner/doom-emacs][doom emacs]]
  ...
  #+end_src
  should work.
- For windows user  =sas-sas-windows= must be set to non-nil (windows SAS flags) and =sas-realsession= must be set to nil (no SAS Interactive Line Mode), for instance using the following code in =.emacs=
   #+begin_src emacs-lisp
   ;; to disable ESS SAS-mode
   (require 'ess)
   (setq auto-mode-alist (delete '("\\.[Ss][Aa][Ss]\\'" . SAS-mode) auto-mode-alist))
